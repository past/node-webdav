...
var Controller = exports.Controller = function(request, response, options) {
    var pathInfo = utils.unescape(uri.parse(request.url).pathname);
    ...
    //    options.scriptName = utils.unescape(request["SCRIPT_NAME"]);
...
Controller.prototype.OPTIONS = function() {
    if (this.options.class2) {
        this.headers["Allow"] = "OPTIONS,HEAD,GET,PUT,POST,DELETE,PROPFIND,PROPPATCH,MKCOL,COPY,MOVE,LOCK,UNLOCK";
        this.headers["Dav"] = "1,2";
    } else {
        this.headers["Allow"] = "OPTIONS,HEAD,GET,PUT,POST,DELETE,PROPFIND,PROPPATCH,MKCOL,COPY,MOVE";
        this.headers["DAV"] = "1";
    }
    this.headers["MS-Author-Via"] = "DAV";
}
...
//XXX
var serializer = undefined;//new libxml.XMLSerializer();
...
Controller.prototype.requestDocument = function() {
    try {
        if (!this.requestDoc) {
            ...
            this.requestDoc = new libxml.DOMParser().parseFromString(body, "text/xml");
...
Controller.prototype.requestMatch = function(pattern) {
    var doc = this.requestDocument(),
        result = libxml.evaluate(pattern, doc, nsMapper, libxml.XPathResult.ORDERED_NODE_ITERATOR_TYPE);
...
Controller.prototype.multistatus = function(block) {
    this.renderXML(function() {
        var doc = libxml.createDocument("DAV:", "D:multistatus", null);
    ...
    for (var status in stats) {
        ...
        props.forEach(function(p) {
            ...
            //if (value) {
                ...
                if (value instanceof libxml.Node || value instanceof libxml.Element)
...
Controller.prototype.mapExceptions = function(block) {
    try {
		...
		console.log("MAPEXCEPTIONS:["+e+"]["+e.message+"]");
        /*
        if (system.verbose) {
